...
if dados:
    ...
    if not df_mes.empty or not df_retroativos.empty:
        ...
        if not df_retroativos.empty:
            linhas_retro = max(1, (len(df_retroativos) // 3) + 1)
            altura_iframe += 400 + (linhas_retro * 400)

        # ── LAÇO MÊS ATUAL (INALTERADO) ───────────────────────────────────────
        cards_html = ""
        ...
        # ── LAÇO RETROATIVO (ESTRUTURA EXACTAMENTE IGUAL) ─────────────────────
        cards_retro_html = ""
        if not df_retroativos.empty:
        ...
        <body>
            ...
            {f'''
            <div class="mural-header" style="margin-top: 100px;">
                <div class="mural-header-inner">
                    <p class="subtitulo">✦ Celebrações Anteriores ✦</p>
                    <h1>Mural Retroativo</h1>
                </div>
            </div>
            <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 40px; width: 100%; max-width: 1200px; margin: 0 auto;">
                {cards_retro_html}
            </div>
